...
int strChar(string str,int k)
{
	...
	if (bigchar == 0 && smallchar == 0)
	{
		return 0;
	}
	else if (bigchar == k || smallchar == k)
	{
		return 10;
	}
...
int strNumber(string str, int k)
{
	int count = 0;
	for (int i = 0; i < k; i++)
	{
...
int strSymbal(string str, int k)
{
	int count = 0;
	for (int i = 0; i < k; i++)
	{
...
//	cout << sizeof(a) << endl;
	...
	while (cin >> str)
	{
		...
		sum2 = strChar(str, k);
		sum3 = strNumber(str, k);
		sum4 = strSymbal(str, k);
		if (sum2 == 10 && sum3 > 0 && sum4 == 0)
		{
			sum5 = 2;
		}
